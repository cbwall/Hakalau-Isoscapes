...
HAK.data$const.growth.age<-HAK.data$DBH.Total/1.0

HAK.data<-HAK.data %>%
  mutate(growth.rate.cm.y = ifelse(Plot=="KP" & Sample=="Koa", 1.2,
         ifelse(Plot=="RK" & Sample=="Koa", 0.6, "NA")))

HAK.data$growth.rate.cm.y<-as.numeric(HAK.data$growth.rate.cm.y)
...
age.m<-aggregate(const.growth.age~Plot, data=HAK.data, mean)
age.sd<-aggregate(const.growth.age~Plot, data=HAK.data, sd)

# Mann-Whitney U-Test
mwu(HAK.data, age, Plot) # signif, p=0.010
```
...
#all plants
plants<-HAK.data[(HAK.data$Sample.Type=="Plants"),]
...
KP.df<-HAK.data[(HAK.data$Plot=="KP"),]; KP.df.plants<-KP.df[(KP.df$Sample.Type=="Plants"),]
RK.df<-HAK.data[(HAK.data$Plot=="RK"),]; RK.df.plants<-RK.df[(RK.df$Sample.Type=="Plants"),]

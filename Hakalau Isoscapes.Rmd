...
*Plot densities were...*  
KP: Koa (18 trees) Rubus (14 plants) per 20 x 35m plot= *0.026 Koa m^-2^, 0.020 Rubus m^-2^*
...
dev.copy(pdf, "figures/indiv.trunk.dbh.pdf", width=4, height=4, encod="MacRoman")
dev.off()
...
Ndfa.mod2<-lm(d15N.Rub~d15N.Koa + Plot, data=Ndfa) # main effects only

anova(Ndfa.mod1, Ndfa.mod2) # effects not different among plots
...
```{r mean plots for DBH and Canopy, Fig.cap="Supplementary Figure 1. Mean +/- SE diameter at breast height (DBH) and *A. koa* canopy area for two afforested and remnant forest plots.", fig.align='center', results='hide', eval=FALSE}
#### plots
...
dev.copy(pdf, "figures/FigS1x.DBHCanopy.pdf", width=4, height=3.5, encod="MacRoman")
dev.off()
```


```{r DBH and Canopy scatte, Fig.cap="Supplementary Figure 1.  Multi-trunk and individual diameter at breast height (DBH) and *A. koa* canopy area for two afforested and remnant forest plots.", fig.align='center', results='hide'}
##### alternative plots

multi.trunk.box<-ggplot(HKP.data, aes(x=Plot, y=DBH.Total, fill=Plot)) + 
...
canopy.box<-ggplot(HKP.data, aes(x=Plot, y=Canopy.area, fill=Plot)) + 
  geom_boxplot(outlier.shape = NA, alpha=0.6)+
  geom_point(pch = 21, position = position_jitterdodge(), alpha=0.6) +
  xlab("Plots") +
  ylab(expression(paste("Canopy (m"^2,")"))) +
...
###################### age
# approx age at 0.7 cm/year
HKP.data$const.growth.age<-HKP.data$DBH.Total/0.7

HKP.data<-HKP.data %>%
  mutate(growth.rate.cm.y = ifelse(Plot=="KP" & Sample=="Koa", 1.2,
         ifelse(Plot=="RK" & Sample=="Koa", 0.6, "NA")))

HKP.data$growth.rate.cm.y<-as.numeric(HKP.data$growth.rate.cm.y)

HKP.data<-HKP.data %>%
  mutate(alt.age = ifelse(Sample=="Koa", DBH.Total/growth.rate.cm.y, "NA"))

HKP.data$alt.age<-as.numeric(HKP.data$alt.age)

#### box plot of mutli-trunks, at 7 mm/y
multi.trunk.altage<-ggplot(HKP.data, aes(y=alt.age, x=Plot, fill=Plot)) + 
  geom_boxplot(outlier.shape = NA, alpha=0.6)+
  geom_point(pch = 21, position = position_jitterdodge(), alpha=0.6) +
  xlab("Plots") +
  ylab("approx. age (dbh cm * 0.7 cm/y)")+
  scale_fill_manual(values=c("#E69F00", "#56B4E9")) +
  annotate(geom="text", x=1.55, y=35, label=expression(paste(bold("*"))), size=5) +
  theme_classic()

multi.trunk.altage 
dev.copy(pdf, "figures/Fig.Sx.multi.trunk.age.pdf", width=4, height=4, encod="MacRoman")
dev.off()

# get some info on the means
age.m<-aggregate(alt.age~Plot, data=HKP.data, mean)
age.sd<-aggregate(alt.age~Plot, data=HKP.data, sd)

# Mann-Whitney U-Test
mwu(HKP.data, age, Plot) # signif, p=0.010
```
...
*Acacia koa* plantation (KP) δ^15^N isoscape
```{r, krig KP d15N, message=FALSE, results='hide'}
## Krig KP sites
...
Remnant Forest (RK) δ^15^N isoscape
```{r, krig RK d15N, message=FALSE, results='hide'}
###########################
# RK Site Krig
###########################
## Krig RK site 
...
```{r, krig KPRK d15N, message=FALSE, results='hide'}
#####################
...
```{r, krig KP d13C, message=FALSE, results='hide'}
...
```{r, krig RK d13C, message=FALSE, results='hide'}
###########################
# RK Site Krig
###########################
## Krig RK site 
...
combine the plots
```{r, krig KPRK d13C, message=FALSE, results='hide'}
#####################
grid.draw.ggmatrix <- function(x, recording = TRUE) {
  print(x)
}
...
```{r, density plots fig S2, fig.show='hold', out.height='50%', message=FALSE, results='hide'}

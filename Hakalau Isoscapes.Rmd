...
pdf(file= "figures/notused/map_cred.pdf", height=5, width=8)
...
```{r, KPRK d13C plants krig, message=FALSE, results='hide'}
#####################
grid.draw.ggmatrix <- function(x, recording = TRUE) {
  print(x)
}
...
```{r, d15N KPRK plants krig, message=FALSE, results='hide'}
#####################
grid.draw.ggmatrix <- function(x, recording = TRUE) {
  print(x)
}
...
```{r, foliar density plots, fig.show='hide', message=FALSE, results='hide'}
# make density plots using predictions
# combine the predictions for each plot
...
```{r, all density plots, fig.show='hold', out.width='50%', message=FALSE, results='hide'}
...
density.plots<-plot_grid(
  density.predict.Nsoil + ylab("density") + theme(legend.position = "none") + fig.format3,

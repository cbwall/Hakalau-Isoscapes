...
Rub.Koa.d15N.plot<-ggplot(data=Ndfa, aes(x=d15N.Koa, y=d15N.Rub, color=Plot))+
  ...
  scale_color_manual(values=c("gray60", "gray85")) +
...
```{r mean plots for DBH and Canopy, Fig.cap="Supplementary Figure 1. Mean +/- SE diameter at breast height (DBH) and *A. koa* canopy area for two afforested and remnant forest plots.", fig.align='center', results='hide'}
#### plots
...
dev.copy(pdf, "figures/Fig 2.mean.data.sample.pdf", width=8.2, height=3, encod="MacRoman")
dev.off()
```
...
density.legend <- get_legend(
  density.predict.C + #plot this one
    theme(legend.title=element_blank())+ # no need for a title above legend
...
density.predict.Nsoil<-ggplot(pred.soil.N, aes(x=d15N.pred)) +
  ...
  geom_vline(data=predict.mean.N, aes(xintercept=grp.mean, color=Plot),
             linetype="dashed", lwd=0.2) + BW.back2 +
  annotate(geom="text", x=2, y=1.2, label=expression(paste(bold("A"))), size=3)
...
density.predict.Csoil<-ggplot(pred.soil.C, aes(x=d13C.pred)) +
  ...
  geom_vline(data=predict.mean.C, aes(xintercept=grp.mean, color=Plot),
             linetype="dashed", lwd=0.2) + BW.back2 +
  annotate(geom="text", x=-28, y=0.8, label=expression(paste(bold("B"))), size=3)
...
dev.copy(pdf, "figures/Fig S4_b.soil.mod.pred.pdf", width = 6, height = 2.5)
dev.off()
...
```{r, plant density plots, fig.show='hold', out.width='50%', message=FALSE, results='hide'}
# make density plots using predictions
# combine the predictions for each plot
...
dev.copy(pdf, "figures/Fig S6_plant.mod.pred.pdf", width = 4, height = 2.5)
dev.off()

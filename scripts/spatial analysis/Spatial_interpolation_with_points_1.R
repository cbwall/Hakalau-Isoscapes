########Plot_bubble_data_followed_by_Kriging 
##from oscarperpinan's book on visualization of spacetime data
##################################################################
...
load('scripts/spatial analysis/NO2sp.RData')

pdf(file="scripts/spatial analysis/airMadrid_spplot.pdf")
...
distritosMadrid <- readShapePoly('scripts/spatial analysis/Distritos/200001494')
...
#streets <- readShapeLines('Callejero_ Ejes de viales/linearenfe.shp')
...
#I was unable to find the madrid streets shapefiles from the book
...
spplot(NO2sp["classNO2"], col.regions=airPal, cex=dentAQ,
       ...
       sp.layout=list(spDistricts, spNames), #spStreets, 
       scales=list(draw=TRUE),
       key.space=NO2key)

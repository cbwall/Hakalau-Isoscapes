...
- d15N differ by PLOT (AK vs RK), no other effects
...
*Figure 1. with afforested and remnant forests*   
...
- perhaps not to be included in paper, but good visual
...
d13C.lm<-lm(d13C~Plot*Sample, data=Hak.data)
d13C.av<-(aov(d13C.lm)); summary(d13C.av) # Plot and Sample effects
plot(allEffects(d13C.lm)) # 2-way interaction
plot(allEffects(lm(d13C~Plot+Sample, data=Hak.data))) # main effects
...
d15N.lm<-lm(d15N~Plot*Sample, data=Hak.data)
d15N.av<-(aov(d15N.lm)); summary(d15N.av) # Plot and Sample effects
plot(allEffects(d15N.lm)) # 2-way interaction
plot(allEffects(lm(d15N~Plot+Sample, data=Hak.data))) # main effects
...
TC.lm<-lm(Total.C..mmol.gdw~Plot*Sample, data=Hak.data)
TC.av<-(aov(TC.lm)); summary(TC.av) # Sample and Plot effects
plot(allEffects(lm(Total.C..mmol.gdw~Plot + Sample, data=Hak.data))) # higher in RK and Koa
tukey<-TukeyHSD(TC.av); tukey

######## ######## 
######## Nitrogen content

TN.lm<-lm(Total.N..mmol.gdw~Plot*Sample, data=Hak.data)
TN.av<-(aov(TN.lm)); summary(TN.av) # Sample effects
plot(allEffects(lm(Total.N..mmol.gdw~Sample, data=Hak.data))) # main effects
tukey<-TukeyHSD(TN.av); tukey

######## ######## 
######## Nitrogen content

CN.lm<-lm(C.N~Plot*Sample, data=Hak.data)
CN.av<-(aov(CN.lm)); summary(CN.av) # Plot Sample effects
plot(allEffects(lm(C.N~Plot +Sample, data=Hak.data))) # main effects
...
## d15N plot
d15N.krig.RK <- krige(d15N ~ 1, krig.RK, GridRK)
...
par(mfrow=c(1,2))
gridExtra::grid.arrange(plotd15N.krig.RK, plotd15N.krig.AK, ncol=2, nrow=1)
dev.copy(pdf, "figures/RKAK.d15N.krig2.pdf", width=7, height=4, encod="MacRoman")
dev.off()

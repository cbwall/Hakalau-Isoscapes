...
Run statistics on the responses (d13C, d15N, tissue N and C)
...
# inspect model
plot(pred.AK.df.N$krige_output.var1.pred ~ pred.AK.df.N$d15N, xlab = "Observed", ylab = "Predicted", cex=0.2)
abline(0,1, lty=2)
abline(mod, col = "blue")
...
# inspect model
plot(pred.RK.df.N$krige_output.var1.pred ~ pred.RK.df.N$d15N, xlab = "Observed", ylab = "Predicted", cex=0.2)
abline(0,1, lty=2)
abline(mod, col = "blue")
...
density plots using predictions of N values
```{r, mod predict}
###################
...
density.predict<-ggplot(pred.dat.N, aes(x=d15N.pred)) +
...
dev.copy(pdf, "figures/Fig S2.mod.pred.pdf", width = 4, height = 2.5)
dev.off()
...
# inspect model
plot(pred.AK.df.C$krige_output.var1.pred ~ pred.AK.df.C$d13C, xlab = "Observed", ylab = "Predicted", cex=0.2)
abline(0,1, lty=2)
abline(mod, col = "blue")
...
mean(pred.RK.df.C$krige_output.var1.pred, na.rm=T) # mean of predictions -27.7 d13C
mean(pred.RK.df.C$krige_output.var1.stdev, na.rm=T) # 2.8

# inspect model
plot(pred.RK.df.C$krige_output.var1.pred ~ pred.RK.df.C$d13C, xlab = "Observed", ylab = "Predicted", cex=0.2)
abline(0,1, lty=2)
abline(mod, col = "blue")
...
density plots using predictions of C values
```{r, mod predict C}
###################
# make density plots using predictions
# combine the predictions for each plot
...
density.predict<-ggplot(pred.dat.C, aes(x=d13C.pred)) +
  geom_density(aes(color=Plot, fill=Plot), alpha=0.5)+
...
print(density.predict)
dev.copy(pdf, "figures/Fig S2b.mod.pred.C.pdf", width = 4, height = 2.5)
dev.off()

```
  
Now make rasters and kirgs just for the Koa and Rubus.

#### AK plot rasters  

Subset data and only examine plants or SOIL. 
d15N
```{r}
###### AK site d15N
...
mean(pred.AK.soil.df.N$krige_output.var1.pred, na.rm=T) # mean of predictions 4.4 d15N
mean(pred.AK.soil.df.N$krige_output.var1.stdev, na.rm=T) # 2.1

# inspect model
plot(pred.AK.soil.df.N$krige_output.var1.pred ~ pred.AK.soil.df.N$d15N, xlab = "Observed", ylab = "Predicted", cex=0.2)
abline(0,1, lty=2)
abline(mod, col = "blue")
...
mean(pred.RK.soil.df.N$krige_output.var1.pred, na.rm=T) # mean of predictions 4.4 d15N
mean(pred.RK.soil.df.N$krige_output.var1.stdev, na.rm=T) # 2.1

# inspect model
plot(pred.RK.soil.df.N$krige_output.var1.pred ~ pred.RK.soil.df.N$d15N, xlab = "Observed", ylab = "Predicted", cex=0.2)
abline(0,1, lty=2)
abline(mod, col = "blue")
...
plotd15N.krig.RK.soil<-spplot(d15N.RK.soil$krige_output["var1.pred"], col.regions=col.scheme.N, 
       sp.layout = list("sp.points", RK.soil, pch=1, 
                        col=c("black"), cex=0.5, alpha=0.5),
       main=list(label=expression(paste(delta^{15}, N, " - RK (Restored Forest)")), cex=0.8), colorkey=TRUE)
plotd15N.krig.RK.soil
...
mean(pred.AK.soil.df.C$krige_output.var1.pred, na.rm=T) # mean of predictions 4.4 d13C
mean(pred.AK.soil.df.C$krige_output.var1.stdev, na.rm=T) # 2.1

# inspect model
plot(pred.AK.soil.df.C$krige_output.var1.pred ~ pred.AK.soil.df.C$d13C, xlab = "Observed", ylab = "Predicted", cex=0.2)
abline(0,1, lty=2)
abline(mod, col = "blue")
...
# inspect model
plot(pred.RK.soil.df.C$krige_output.var1.pred ~ pred.RK.soil.df.C$d13C, xlab = "Observed", ylab = "Predicted", cex=0.2)
abline(0,1, lty=2)
abline(mod, col = "blue")
...
plotd13C.krig.RK.soil<-spplot(d13C.RK.soil$krige_output["var1.pred"], col.regions=col.scheme.C, 
       sp.layout = list("sp.points", RK.soil, pch=1, 
                        col=c("black"), cex=0.5, alpha=0.5),
       main=list(label=expression(paste(delta^{13}, C, " - RK (Restored Forest)")), cex=0.8), colorkey=TRUE)
plotd13C.krig.RK.soil
...
TheVariogramModel <- vgm(psill=0.6, model="Gau", nugget=0, range=4)
...
spplot(d15N.KOA.AK.auto$krige_output,"var1.pred", col.regions=col.scheme.C, 
       sp.layout = list("sp.points", krig.RK, pch=c(16,17,1)[krig.RK$Sample],
                        col="black", cex=0.5, alpha=0.5), 
       main=list(label=expression(paste(delta^{13}, C, " - RK (Remnant Forest)")), cex=0.7))

#predicted krige
# other way to call it in to spplot --  d15N.KOA.AK.auto$krige_output["var1.pred"]
...
plotd15N.Soil.krig.RK2<-spplot(d15N.Soil.RK2["var1.pred"], col.regions=colorRampPalette(col.scheme.N), 
       main=expression(paste(delta^{15}, N, "--RK (Remnant Soil Plot)")))

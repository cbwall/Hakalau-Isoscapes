...
AK.map<-krig.AK %>% as.data.frame %>% 
  ggplot(aes(x.meter, y.meter)) + geom_point(aes(size=d15N), color="forestgreen", alpha=3/4) + 
...
RK.map<-krig.RK %>% as.data.frame %>% 
  ggplot(aes(x.meter, y.meter)) + geom_point(aes(size=d15N), color="dodgerblue", alpha=3/4) +
  ggtitle("RK plot--d15N (permil)") + coord_equal() + theme_bw()


plot_grid(RK.map, AK.map, ncol=2, nrow=1)
dev.copy(pdf, "figures/dot.scape.pdf", width=7, height=4, encod="MacRoman")
dev.off()


coordinates(krig.AK)<- ~x.meter + y.meter
str(krig.AK)
vario.dir1 <- variogram(d15N~x.meter+y.meter, krig.AK, alpha=c(0,45,90,135))

vario.dir2 <- variogram(d15N~1, krig.AK, width=90, cutoff=1300)
vario.fit <- fit.variogram(vario.dir2, vgm(1, "Sph", 900, 1)) # fit model
...
test.krig<-krige(d15N~x.meter+y.meter, krig.AK, grid, model=vario.fit)

AK.isoscape<- test.krig %>% as.data.frame %>%
...
grid<-makegrid(krig.RK, n=2000)
colnames(grid)<-c('x.meter', 'y.meter')
grid<-as.data.frame(grid)

#where we have measurements
plot1 <- krig.RK %>% as.data.frame %>%
  ggplot(aes(x.meter, y.meter)) + geom_point(size=1) + coord_equal() + 
  ggtitle("Points with measurements")
...
# fit the model for the krig object (grid) and use model fit from corresponding model data above
test.krig<-krige(d15N~x.meter+y.meter, krig.RK, grid, model=vario.fit)

RK.isoscape<-test.krig %>% as.data.frame %>%
  ggplot(aes(x=x.meter, y=y.meter)) + geom_tile(aes(fill=var1.pred)) + coord_equal() +
  scale_fill_gradient(low = "yellow", high="red") +
  ggtitle("RK isoscape") +
  scale_x_continuous(labels=comma) + scale_y_continuous(labels=comma) +
...
grid.arrange(AK.isoscape, RK.isoscape, ncol = 2)
dev.copy(pdf, "figures/isoscape.plots.pdf", width=7, height=7, encod="MacRoman")
dev.off()

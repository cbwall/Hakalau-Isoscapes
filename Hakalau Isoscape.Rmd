...
# DBH for trees with single trunk
DBH.single <-


# DBH for trees with multiple trunks
iso.dat$DBH1<-(iso.dat$DBH..cm.1^2)
iso.dat$DBH2<-(iso.dat$DBH..cm.2^2)
iso.dat$DBH3<-(iso.dat$DBH..cm.3^2)
iso.dat$DBH4<-(iso.dat$DBH..cm.4^2)
iso.dat$DBH5<-(iso.dat$DBH..cm.5^2)

# replace NAs with zeros to solve sqrt problem
iso.dat<- iso.dat %>%
  mutate_each(funs(replace(., which(is.na(.)), 0)))

# sqrt and sum the sum of squares for individual trunk DBH
iso.dat$DBH.Total<-sqrt(iso.dat$DBH1 + iso.dat$DBH2 + iso.dat$DBH3 + iso.dat$DBH4 + iso.dat$DBH5)


iso.data<- iso.dat %>%
  select(c(Plot, Sample, d15N, d13C, Total.N..mmol.gdw, Total.C..mmol.gdw, C.N, DBH.Total))
...
plotd15N.koa.krig.AK<-spplot(d15N.KOA.AK["var1.pred"], col.regions=colorRampPalette(col.scheme.N), 
       main=expression(paste(delta^{15}, N, "--AK (Afforested Koa Plot)")))



koa.RK<-  krig.RK %>%
  mutate(d15N=replace(d15N, Sample=="Soil", NA))

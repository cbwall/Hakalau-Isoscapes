...
# load packages
pacman::p_load("RgoogleMaps", "SDMTools", "rgdal", "ggmap", "gridExtra", "automap", "cowplot","sp", "gstat", "plotrix", "dplyr", "ggplot2", "scales","magrittr","plyr","effects", "ggpubr", "sjstats")
...
- the degree of nitrogen fixation takes into account the target nitrogen fixer (Koa) and a non-fixing reference plant (Rubus spp.) and Beta. Beta is the value when all nitrgen % in fixing plat comes from nitrogen fixation and is estimated, usually as -2 to 0. The formula is:  
$Ndfa (\%) = (δ^{15}N~ref~ - δ^{15}N~fixer~) / (δ^{15}N~ref~ - Beta)* 100$   

Here we use Rubus maximum, site median, and group media d15N values as reference plants and a Beta of -1 and -2 to see the % Ndfa of Koa across its range of d15N values.   
...
d13C.plot<-ggplot(data=mean.data, aes(x=Plot, y=d13C.mean, fill=Sample, group=Sample))+
  ...
  scale_fill_manual(values=c('skyblue1','mediumseagreen', "goldenrod")) +
  xlab("Plots") + 
  annotate(geom="text", x=1.5, y=-30, label=expression(paste(bold("*"))), size=4) +
...
d15N.plot<-ggplot(data=mean.data, aes(x=Plot, y=d15N.mean, fill=Sample, group=Sample))+
  ...
  xlab("Plots") + 
  theme(text = element_text(size=8)) +
  annotate(geom="text", x=1.5, y=5, label=expression(paste(bold("*"))), size=4) +
...
dev.copy(pdf, "figures/Supp.Fig2.mean.data.plots.pdf", width=7, height=3, encod="MacRoman")
dev.off()
```

...
plants samples were collected in a spatially explicit fashion (every 4 or 5m) with a 20 x 35m superplot, consisting of thirty-five 4 x 5m subplots. This data was used to create a δ^13^C and δ^15^N stable isotope landscape, hereafter 'isoscape'. Koa and *Rubus* spp. were collected within each 4 x 5m sublot.  
...
########## Separate dataframes
plants<-Hak.data[(Hak.data$Sample=="plants"),]
Koa<-Hak.data[(Hak.data$Sample=="Koa"),]
Rubus<-Hak.data[(Hak.data$Sample=="RUBARG" | Hak.data$Sample=="RUBHAW"),]
AK.df<-Hak.data[(Hak.data$Plot=="AK"),]; AK.df.sanplants<-AK.df[!(AK.df$Sample=="plants"),]
RK.df<-Hak.data[(Hak.data$Plot=="RK"),]; RK.df.sanplants<-RK.df[!(RK.df$Sample=="plants"),]
...
d15N.AK<-lm(d15N~Sample, data=AK.df.sanplants); anova(d15N.AK) #only koa vs. rubus comparision, signif.
plot(allEffects(d15N.AK)) 

d15N.RK<-lm(d15N~Sample, data=RK.df.sanplants); anova(d15N.RK) #only koa vs. rubus comparision, signif.
...
d13C.AK<-lm(d13C~Sample, data=AK.df.sanplants); anova(d13C.AK) #only koa vs. rubus comparision
plot(allEffects(d13C.AK))

d13C.RK<-lm(d13C~Sample, data=RK.df.sanplants); anova(d13C.RK) #only koa vs. rubus comparision
...
N.AK<-lm(Total.N..mmol.gdw~Sample, data=AK.df.sanplants); anova(N.AK) #only koa vs. rubus comparision, signif.
plot(allEffects(N.AK)) 

N.RK<-lm(Total.N..mmol.gdw~Sample, data=RK.df.sanplants); anova(N.RK) #only koa vs. rubus comparision, signif.
...
C.AK<-lm(Total.C..mmol.gdw~Sample, data=AK.df.sanplants); anova(C.AK) #only koa vs. rubus comparision, signif.
plot(allEffects(C.AK)) 

C.RK<-lm(Total.C..mmol.gdw~Sample, data=RK.df.sanplants); anova(C.RK) #only koa vs. rubus comparision, signif.
...
CN.AK<-lm(C.N~Sample, data=AK.df.sanplants); anova(CN.AK) #only koa vs. rubus comparision, signif.
plot(allEffects(CN.AK)) 

CN.RK<-lm(C.N~Sample, data=RK.df.sanplants); anova(CN.RK) #only koa vs. rubus comparision, not signif
...
mean.data$Sample<-factor(mean.data$Sample, levels=c("plants", "Koa", "Rubus spp"))
...
# autokrige
d15N.AK.plants <- autoKrige(d15N ~ 1, AK.plants, Grid.AK) # ordinary kriging
...
mean(pred.AK.plants.df.N$krige_output.var1.pred, na.rm=T) # mean of predictions 6.0 d15N
mean(pred.AK.plants.df.N$krige_output.var1.stdev, na.rm=T) # 0.35
...
dev.copy(png, "figures/RKAK.d15N.plantss.png", width = 8, height = 4, units = 'in', res = 300)
dev.off()

```

#### d13C plants isoscape
```{r}
###### AK site d13C
...
mean(pred.AK.plants.df.C$krige_output.var1.pred, na.rm=T) # mean of predictions -24.4 d13C
mean(pred.AK.plants.df.C$krige_output.var1.stdev, na.rm=T) # 0.6
...
mean(pred.RK.plants.df.C$krige_output.var1.pred, na.rm=T) # mean of predictions 4.4 d13C
mean(pred.RK.plants.df.C$krige_output.var1.stdev, na.rm=T) # 2.1
...
dev.copy(png, "figures/RKAK.d13C.plantss.png", width = 8, height = 4, units = 'in', res = 300)
dev.off()
...
```{r, plant density plots, fig.show='hold', out.width='50%'}
# make density plots using predictions
# combine the predictions for each plot
...
gridExtra::grid.arrange(density.predict.Nplants)
dev.copy(pdf, "figures/Fig Sx.plant.mod.pred.pdf", width = 5, height = 2.5)
dev.off()
